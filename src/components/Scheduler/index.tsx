...
const SchedulerComp = () => {
  ...
  const events = bookingList?.map((item) => {
    ...
    const startDate = new Date(
      new Date(item.datetime.date.toDate().toDateString()).setHours(
        startHour,
        startMinute,
        0
      )
        ...
        viewerExtraComponent={(_, event) => {
          return (
            <div>
              ...
              <div>
                - Thời gian:{' '}
                {moment(event.start?.toString()).format('DD/MM/YYYY')}{' '}
                {getTimeRange(event?.datetime?.time)}
              </div>
              <div>
                - Gói dịch vụ:{' '}
                {event.services
              ...
              <div>
                - Trạng thái:{' '}
                {event.status === STATUS.DONE ? 'Đã cắt' : 'Chưa cắt'}
              </div>
              <div className='mt-3 mb-3'>
                <button
                  className=' bg-[#9f6e0dd4] text-white px-3 py-1 rounded outline-none focus:outline-none m-auto select-btn-barber'
                  onClick={() =>
                    handleDoneItem(event.id, event.status || STATUS.OPEN)
                  }
                >
...
const CustomEditor = ({ scheduler }: CustomEditorProps) => {
  ...
  const finalUpdate = async (data: any) => {
    ...
    const startDate = new Date(
      newState.datetime.date.setHours(startHour, startMinute, 0)
    );

    const endDate = new Date(
      newState.datetime.date.setHours(endHour, endMinute, 0)
    );
      ...
      <div style={{ padding: '1rem' }}>
        ...
        {SERVICES?.map((item) => (
          <div key={item.id}>
            <input
              ...
              defaultChecked={state.services?.find(
                (s: any) => s.id === item.id
              )}
        ...
        {!clickedOnBarber && (
          <div className='align-right mt-3'>
            <button
              className='border-gray-300 border mr-2 px-4 py-2 rounded outline-none focus:outline-none select-btn-barber'
              onClick={scheduler.close}
            >
              Hủy
            </button>
